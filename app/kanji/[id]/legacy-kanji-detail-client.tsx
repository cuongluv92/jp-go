...
function legacyKanjiDetailCacheKey(id: string): string {
  return `legacy-kanji-detail-${id}`;
}
...
export function LegacyKanjiDetailClient({ id }: { id: string }) {
  const cached = getCached<KanjiDetailCachedData>(legacyKanjiDetailCacheKey(id));
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setDetail(data);
      setLoading(false);
      setCached<KanjiDetailCachedData>(legacyKanjiDetailCacheKey(id), { detail: data, userId: user?.id ?? null });
    }
    void load();
    return () => {
      cancelled = true;

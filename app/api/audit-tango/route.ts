...
function groupedDuplicates<T>(rows: T[], keyOf: (row: T) => string, previewOf: (row: T) => unknown) {
...
export async function GET(request: NextRequest) {
  ...
  const badCoverage = sampleWords
    .map((word) => {
      ...
      return { id: word.id, word: word.word, count: rows.length, types, nos };
    })
    .filter((row) => row.count !== 3 || row.types.join("|") !== "business|daily|exam" || row.nos.join("|") !== "1|2|3");
  ...
  const emptyWordFields = sampleWords
    .map((word) => ({
      id: word.id,
      word: word.word,
      empty: [!word.word && "word", !word.reading && "reading", !word.meaningVi && "meaningVi", !word.partOfSpeech && "partOfSpeech"].filter(Boolean),
    }))
  ...
  const emptyExampleFields = sampleExamples
    .map((example, index) => ({
      ...
      vocabId: example.vocabId,
      exampleNo: example.exampleNo,
      empty: [!example.exampleJp && "exampleJp", !example.exampleVi && "exampleVi", !example.clozeJp && "clozeJp", !example.answer && "answer"].filter(Boolean),
    }))
    .filter((row) => row.empty.length > 0);

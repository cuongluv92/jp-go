          ...
          <li key={item.href}>
            <Link
              ...
              className={`group flex items-center gap-3 rounded-2xl px-3 py-2.5 transition ${
                isActive ? "bg-accent-soft text-accent" : "text-foreground hover:bg-slate-50 dark:hover:bg-surface-muted"
              }`}
            >
              <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${isActive ? "bg-white dark:bg-surface text-accent shadow-sm" : "bg-slate-100 dark:bg-white/10 text-muted group-hover:text-foreground"}`}>
                {item.icon}
              </span>
  ...
  if (desktopMode) {
    return (
      ...
      >
        ...
        <div className="mt-3 shrink-0 rounded-2xl bg-slate-50 dark:bg-surface-muted px-3 py-3">
          <p className="text-xs font-semibold text-foreground">Phím tắt nhanh</p>
          <div className="mt-2 flex items-center justify-between text-[10px] text-muted">
            <span>Tìm mọi thứ</span>
            <kbd className="rounded-md border border-border bg-white dark:bg-surface px-1.5 py-0.5 font-semibold">Ctrl K</kbd>
          </div>
          <div className="mt-1.5 flex items-center justify-between text-[10px] text-muted">
            <span>Tìm nhanh</span>
            <kbd className="rounded-md border border-border bg-white dark:bg-surface px-1.5 py-0.5 font-semibold">/</kbd>
          </div>
        </div>
      </aside>
    );
            ...
            <li key={item.href} className="flex justify-center py-1.5">
              <Link
                ...
                className={`flex flex-col items-center gap-0.5 rounded-2xl px-3.5 py-1.5 text-[11px] font-medium transition-colors ${
                  isActive ? "bg-accent-soft text-accent" : "text-muted active:bg-slate-100 dark:active:bg-white/10"
                }`}
                aria-current={isActive ? "page" : undefined}
              >
                {item.icon}

...
export function renderBlockColumns(block: ContentBlock, showFurigana = false): BlockColumns {
  switch (block.type) {
    ...
    case "formula": {
      const formulaPre = (
        <pre className="min-w-0 flex-1 overflow-x-auto rounded-xl bg-slate-900 px-4 py-3 font-mono text-[15px] leading-[1.4] tracking-normal text-slate-50 dark:text-foreground">
          {block.content}
        </pre>
      );
      return {
